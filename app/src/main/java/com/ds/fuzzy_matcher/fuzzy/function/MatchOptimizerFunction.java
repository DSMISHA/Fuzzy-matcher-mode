package com.ds.fuzzy_matcher.fuzzy.function;

import com.ds.fuzzy_matcher.fuzzy.domain.Match;
import com.ds.fuzzy_matcher.fuzzy.domain.NGram;
import com.ds.fuzzy_matcher.fuzzy.domain.Token;
import com.ds.fuzzy_matcher.fuzzy.exception.MatchException;
...
public interface MatchOptimizerFunction extends Function<List<Token>, Stream<Match<Token>>> {
    ...
    static Stream<List<Token>> getGroupsByNGram(List<Token> input) {
        Stream<NGram> nGramStream = input.parallelStream().flatMap(new Function<Token, Stream<? extends NGram>>() {
            @Override
            public Stream<? extends NGram> apply(Token token) {
                return token.getNGrams();
            }
        });
        ...
        return ngramByTokenMap.values().stream().distinct().filter(new Predicate<List<Token>>() {
            @Override
